...
class Game:
        ...
        for row in range(self.board_size):
            for col in range(self.board_size):
                ...
                if self.symbols_on_table[row][col] == "x":
                    pygame.draw.line(self.window.window, (200, 0, 0), (cell_x + 50, cell_y + 50), (cell_width - 50, cell_height - 50), width=3)
                    pygame.draw.line(self.window.window, (200, 0, 0), (cell_width - (cell_x + 50) + cell_x, cell_y + 50), (cell_width - (cell_width - 50) + cell_x, cell_height - 50), width=3)
                elif self.symbols_on_table[row][col] == "o":
                    pygame.draw.circle(self.window.window, (0, 0, 200), ((cell_x + cell_width) / 2, (cell_y + cell_height) / 2), 50, width = 3)

        for width_multiplier in range(self.board_size):
            pygame.draw.line(self.window.window, (0, 0, 0), (self.window.width / self.board_size * width_multiplier, 0), (self.window.width / self.board_size * width_multiplier, self.window.height), width=3)
        for height_multiplier in range(self.board_size):
            pygame.draw.line(self.window.window, (0, 0, 0), (0, self.window.height / self.board_size * height_multiplier), (self.window.width, self.window.height / self.board_size * height_multiplier), width=3)
    ...
    def _check_winning_position(self) -> bool:
        ...
        for row in self.symbols_on_table:
            ...
            for symbol in self.symbols_on_table[row][1:]:
                if self.symbols_on_table[row][0] == "" or (self.symbols_on_table[row][0] != symbol or symbol == ""):
                    win_in_row = False
                    continue
            if win_in_row:
                return True
        ...
        # 3. Check diagonals
        left_to_right_diagonal = [self.symbols_on_table[row][row] for row in self.symbols_on_table]
        if "" not in left_to_right_diagonal:
            if all([left_to_right_diagonal[0] == symbol for symbol in left_to_right_diagonal[1:]]):
                return True
            
        right_to_left_diagonal = [self.symbols_on_table[row][self.board_size - row - 1] for row in self.symbols_on_table]
    ...
    def _show_winner(self, is_win: bool, is_full: bool) -> bool:
        ...
        if is_win:
            ...
            for i in range(3):
                label = self.font.render(f"{self.next_symbol} is the winner!", 1, (0, 0, 0))
                label2 = self.font.render(f"Exiting in {3 - i}", 1, (0, 0, 0))
                self.window.window.blit(label, ( (self.window.width / 2) - (label.get_width() / 2), (self.window.height / 2) - (label.get_height() / 2) ) )
                self.window.window.blit(label2, ( (self.window.width / 2) - (label2.get_width() / 2), (self.window.height / 2) - (label2.get_height() / 2) + label2.get_height() ) )
                pygame.display.flip()
                time.sleep(1)
                self.window.window.fill((127, 127, 127))
